...
data class HabitUiState(
    ...
    val streakHabits: List<HabitWithCompletions> = emptyList(),
...
class HabitViewModel @Inject constructor(private val repository: HabitRepository) : ViewModel() {
    ...
    // Bounded rolling history for the aggregate streak card. Keeping this separate avoids
    // loading an unbounded completion history into the UI layer.
    private val streakFlow = selectedDate.flatMapLatest { today ->
        repository.observeForRange(today.minusDays(730), today)
    }
    val uiState: StateFlow<HabitUiState> = combine(todayFlow, weeklyFlow, monthlyFlow, streakFlow, selectedTab) { today, week, month, streak, tab ->
        HabitUiState(
            ...
            try {
                when (repository.toggle(habitId, date, currentlyDone)) {
                    ...
                    ToggleResult.RETROACTIVE_DONE -> {
                        ...
                        toasts.send("Logged retroactively for $fmt")
                    }
                    ToggleResult.FUTURE_BLOCKED ->
                        toasts.send("You can't mark future habits as done yet.")
                    ToggleResult.NOT_SCHEDULED ->
                        toasts.send("This habit isn't scheduled for that day.")
                    ToggleResult.HABIT_NOT_FOUND ->
                        errors.send("Habit not found. Please try again.")
                }
            } catch (e: CancellationException) { throw e }
            catch (e: Exception) { errors.send("Couldn't update this habit. Please try again.") }
        }
    }
